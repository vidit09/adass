...
def make_data_loader(cfg, is_train=True, distributed=False, max_iter=None, start_iter=0):
    ...
    datasets = build_dataset(dataset_list, transform=train_transform, target_transform=target_transform, is_train=is_train, is_da=cfg.withda)
    ...
    for dataset in datasets:
        ...
        # data_loader = DataLoader(dataset, num_workers=1, batch_sampler=batch_sampler,
        #                          pin_memory=cfg.DATA_LOADER.PIN_MEMORY, collate_fn=BatchCollator(is_train))
        data_loaders.append(data_loader)

    if is_train and not cfg.withda:
